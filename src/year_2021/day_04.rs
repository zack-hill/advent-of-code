...
pub fn solve_part_1(input: &Input) -> u32 {
    let mut numbers: Vec<u8> = input.numbers.iter().rev().cloned().collect();
    let mut called_numbers = vec![];

    while let Some(number) = numbers.pop() {
        called_numbers.push(number);
        for board in input.boards.iter() {
...
pub fn parse_input() -> Input {
    let file = File::open("src/year_2021/day_04.txt").unwrap();
    ...
    let input = Input { numbers, boards };
    return input;
}
